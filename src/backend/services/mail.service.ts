...
function orderDetailsList(details: Array<{ label: string; value: string }>) {
    ...
    return `<table style="width:100%; border-collapse:collapse; margin-top:18px; font-size:14px;">${items}</table>`;
}
...
export const mailService = {
    async sendRegistrationThankYouEmail({ email, firstName }: RegistrationThankYouEmailInput) {
        ...
        const text = [
            greeting(firstName),
            "",
            `Thank you for registering with ${ENV.WEBSITE_NAME}. Your account is now active.`,
        ...
        const html = baseHtmlTemplate(
            subject,
            `${greeting(firstName)} Thank you for registering with ${ENV.WEBSITE_NAME}. Your account has been created successfully.`,
            `<p style="margin:0; font-size:15px; line-height:1.7;">We appreciate your trust in ${ENV.COMPANY_NAME}. You can now sign in and start using the platform.</p>`
        );
    ...
    async sendOrderConfirmationEmail({
        ...
        const finalDetails = [
            ...
            { label: "Tokens deducted", value: String(tokensDeducted) },
            { label: "Order date", value: orderDate.toISOString() },
        ...
        const text = [
            greeting(firstName),
            "",
            "Your order was created successfully and tokens were deducted successfully.",
            ...finalDetails.map((detail) => `${detail.label}: ${detail.value}`),
        ].join("\n");

        const html = baseHtmlTemplate(
            subject,
            `${greeting(firstName)} Your order was received successfully and the token deduction has been completed.`,
            orderDetailsList(finalDetails)
        );

        await safeSendEmail("sendOrderConfirmationEmail", email, subject, text, html);
    },
};
